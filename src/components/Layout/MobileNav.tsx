...
import { Compass, TrendingUp, Search, Library } from "lucide-react"
import { MOBILE_TABS } from "../../utils/constants"
import type { LucideIcon } from "lucide-react"

const iconMap: Record<string, LucideIcon> = { Compass, TrendingUp, Search, Library }
      ...
      {MOBILE_TABS.map((tab) => {
        ...
        return (
          <NavLink key={tab.path} to={tab.path}
            ...
            <span className={`text-[10px] font-medium ${isActive ? "" : ""}`}>{tab.label}</span>
          </NavLink>
        )
      })}
    </nav>
